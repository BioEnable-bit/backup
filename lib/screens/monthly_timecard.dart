...
import 'package:pcmc_staff/models/MonthlyTimeCard.dart';
...
class MonthlyTimeCard extends StatefulWidget {
  ...
  @override
  State<MonthlyTimeCard> createState() => _MonthlyTimeCardsState();
}

class _MonthlyTimeCardsState extends State<MonthlyTimeCard> {
  List<MonththlyTimeCard> attendanceList = <MonththlyTimeCard>[];
  ...
  Future<List<MonththlyTimeCard>> getmonthlyTimecarddata() async {
    ...
    Response response = await get(
      Uri.parse(
          'https://pcmc.bioenabletech.com/api/service.php?q=monthly_timecard&auth_key=PCMCS56ADDGPIL&staff_id=8908&from_date=$_selectedFromDate&to_date=$_selectedToDate'),
    );
    ...
    //
    // });
    return data.map((e) => MonththlyTimeCard.fromJson(e)).toList();
  }
  ...
  Future getEmployeeNamesformonth() async {
    final prefs = await SharedPreferences.getInstance();
    var staffID = prefs.getString('staffID');
  ...
  void initState() {
    super.initState();
    getEmployeeNamesformonth();
    getmonthlyTimecarddata();
  }
                ...
                ? FutureBuilder(
                    ...
                    builder: (context, data) {
                      if (data.hasError) {
                        ...
                        var items = data.data as List<MonththlyTimeCard>;
                        return Expanded(
                                ...
                                return DataTable(
                                  ...
                                  rows: [
                                    ...
                                    DataRow(cells: [
                                      ...
                                      DataCell(Text(
                                          items[index].in_punch.toString())),
                                      DataCell(Text(
                                          items[index].out_punch.toString())),
                                      DataCell(Text(
                                          items[index].total_hrs.toString())),
                                    ]),
                                  ],
                                );
                              }),
